...
function buildHero(root: HTMLElement) {
  ...
  pin(scene, 150, "v2-hero", (tl) => {
    ...
    tl.to(
      gridScales,
      { scale: () => 1 / sPass(), duration: 20, ease: "power1.in" },
      80,
    );
...
function buildInside(root: HTMLElement) {
  ...
  pin(scene, 140, "v2-inside", (tl) => {
    ...
    if (mfrag)
      tl.fromTo(mfrag, { yPercent: 4 }, { yPercent: -10, duration: 100 }, 0);
...
function buildDisc(root: HTMLElement, isMobile: boolean) {
  ...
  const flood = () =>
    Math.max(
      VW() / Math.max(1, spanR.width),
      VH() / Math.max(1, spanR.height),
    ) * 2.4;
  ...
  pin(scene, isMobile ? 200 : 220, "v2-work", (tl) => {
    ...
    if (isMobile) {
      ...
      units.forEach((u, i) => {
        ...
        if (i < 2)
          tl.to(u, { y: () => -VH() * 0.9, duration: 10 }, enters[i] + 28);
      ...
      units.forEach((u, i) => {
        ...
        if (i < 2)
          tl.to(u, { x: () => -VW() * 1.1, duration: 10 }, enters[i] + 28);
        else tl.to(u, { autoAlpha: 0, duration: 2 }, 84);
      });
    }
    ...
    tl.fromTo(
      threeSpan,
      { scale: 1 },
      { scale: flood, duration: 13, ease: "power1.in" },
      84,
    );
...
function buildPractice(root: HTMLElement) {
  ...
  pin(scene, 260, "v2-practice", (tl) => {
    ...
    tl.to(
      [kick(beats.make), copy(beats.make)],
    ...
    tl.to(
      mplanes,
      { y: () => VH() * 0.42, duration: 14, ease: "power1.inOut" },
      72,
    );
    tl.to(
      [kick(beats.measure), copy(beats.measure)],
      { autoAlpha: 1, duration: 6 },
      76,
    );
    // Settle
...
function buildIndex(root: HTMLElement) {
  ...
  pin(scene, 180, "v2-index", (tl) => {
    ...
    rowEls.forEach((_, i) => {
      ...
      tl.to(
        inv,
        { clipPath: "inset(0% 0% 0% 0%)", duration: w },
        Math.max(0, center - rowH * 0.16),
      );
      // The last row stays inverted — it compresses into the hairline.
      if (i < n - 1) {
        tl.to(
          inv,
          { clipPath: "inset(0% 0% 0% 100%)", duration: w },
          center + rowH * 0.5,
        );
      }
    });
    ...
    tl.to(
      rows,
      { scaleY: 0.015, autoAlpha: 0, duration: tail * 0.7, ease: "power1.in" },
      travel + tail * 0.1,
    );
...
function buildClimax(root: HTMLElement, isMobile: boolean) {
  ...
  pin(scene, 210, "v2-climax", (tl) => {
    ...
    tl.to(
      lock,
      { scaleY: 0.012, autoAlpha: 0, duration: 10, ease: "power1.in" },
      78,
    );
    ...
    tl.to(
      name,
      { autoAlpha: 1, scale: 1, duration: 10, ease: "power1.out" },
      86,
    );
  ...
  qa("[data-v2-anchor]").forEach((a) => {
    a.addEventListener("click", (e) => {
      ...
      gsap.to(window, {
        ...
        ease: "power2.inOut",
        scrollTo: {
          ...(typeof target === "number" ? { y: target } : target),
          autoKill: true,
        },
      });
    });
...
function runLoader(nav: HTMLElement | null) {
  ...
  tl.to(
    mark,
    { scale: 2.3, autoAlpha: 0, duration: 0.38, ease: "power2.in" },
    0.88,
  );
...
async function boot() {
  ...
  const safety = (window as unknown as { __v2loaderTimer?: number })
    .__v2loaderTimer;
