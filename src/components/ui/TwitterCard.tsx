...
export const TwitterCard = ({
  ...
  const [profile, setProfile] = useState({
    name: toPlainText(name) || "Twitter User",
    avatarUrl: avatarUrl || "",
    bannerUrl: "",
    ...
    fetch(`https://api.fxtwitter.com/${username}`)
      ...
      .then((data) => {
        if (data.code === 200 && data.user) {
          ...
          const newProfile = {
            name: toPlainText(user.name || name),
            avatarUrl:
            ...
            location: toPlainText(user.location || ""),
            website: user.website
              ? {
                  ...user.website,
                  display_url: toPlainText(user.website.display_url),
                }
              : null,
          };
  ...
  if (staticCard) {
    return (
      <div
        className={cn(
          "w-80 rounded-2xl border border-dashed border-border bg-bg/95 p-4 shadow-xl backdrop-blur-md transition-colors",
          className,
        )}
      >
        <div className="relative -mx-4 -mt-4 h-24 overflow-hidden rounded-t-2xl bg-neutral-100 dark:bg-neutral-900">
          {profile.bannerUrl ? (
            ...
            <div className="h-full w-full bg-gradient-to-r from-chip to-chip-hover" />
          )}
        </div>
        ...
        <div className="relative mb-2 flex items-start justify-between">
          <img
            ...
            className="relative z-10 -mt-8 h-16 w-16 rounded-full border-4 border-bg object-cover shadow-md"
          />
