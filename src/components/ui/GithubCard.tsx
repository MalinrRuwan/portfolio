...
/**
 * This site follows the OS color scheme via light-dark() (no .dark class), so
 * detect dark mode with a media query instead of a theme provider.
 */
...
export const GithubCard = ({
  ...
  useEffect(() => {
    ...
    fetch(`https://api.github.com/users/${username}`)
      ...
      .then((data) => {
        setProfile({
          name: toPlainText(data.name || data.login || name),
          avatarUrl:
            data.avatar_url ||
            avatarUrl ||
    ...
    <div className={cn("flex items-center gap-1.5", className)}>
      ...
      >
        ...
        <motion.div
          ...
          className={cn(
            "absolute bottom-full z-50 mb-4 w-80 rounded-2xl border border-dashed border-border bg-bg/95 p-6 shadow-xl backdrop-blur-md transition-colors after:absolute after:top-full after:left-0 after:h-4 after:w-full",
            popoverClassName,
          )}
        >
          <div className="mb-4 flex items-center gap-4">
            <img
              ...
              className="h-12 w-12 rounded-full border border-border object-cover shadow-sm dark:shadow-none"
            />
