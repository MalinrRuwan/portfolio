...
export interface BendOptions {
  ...
  tilt?: number;
  /** Optional keyboard key that toggles the bend effect on and off. */
  toggleKey?: string;
}
...
export function createBend(
  ...
  function onClick(event: MouseEvent) {
    ...
    if (
      ...
      directTarget.closest("dialog[open]")
    ) {
      return;
    }
...
export function Bend({
  ...
  const [enabled, setEnabled] = useState(() => {
    if (!toggleKey || typeof window === "undefined") return false;

    try {
      return window.localStorage.getItem(BEND_ENABLED_STORAGE_KEY) === "true";
    } catch {
      return false;
    }
  });
  ...
  return (
    <div
      className={`canvasui-bend-enabled ${className ?? ""}`}
    ...
    >
      ...
      >
        {native ? (
          <div
            ref={contentRef}
            className="canvasui-bend-content"
            style={{
              position: "relative",
              width: "100%",
              height: "100%",
