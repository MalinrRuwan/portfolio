...
function loadImage(source: string): Promise<HTMLImageElement> {
	return new Promise((resolve, reject) => {
		...
		image.onerror = () => reject(new Error("The featured image could not be loaded."));
		image.src = source;
	});
}
...
function drawMetadataRule(context: CanvasRenderingContext2D, ruleY: number): void {
...
async function drawStory(
	...
	const overlay = context.createLinearGradient(0, imageHeight - 250, 0, imageHeight);
...
function EntrySection({ title, entries, selected, onSelect }: EntrySectionProps) {
...
export function StatusImagesPage() {
	...
	useEffect(() => {
		...
		fetch("/_emdash/api/plugins/portfolio-status-images/entries", {
			...
			.catch(() => {
				if (active) {
					setError("The content list could not be loaded. Refresh and try again.");
				}
			})
			.finally(() => {
				if (active) {
		...
		canvas.toBlob(
			(blob: Blob | null) => {
				if (!blob) {
					...
					setError("The image could not be generated. Try another featured image.");
					return;
				}
	...
	const visibleEntries = normalizedSearch
		? entries.filter((entry) =>
				`${entry.title} ${entry.summary}`.toLocaleLowerCase().includes(normalizedSearch),
			)
		: entries;
	const projects = visibleEntries.filter((entry) => entry.collection === "projects");
	...
	return (
		<section className="status-images">
			...
			<header className="status-images__header">
				...
				<h1 style={{ margin: 0, color: INK, fontSize: 32 }}>Status image generator</h1>
				<p style={{ color: MUTED, maxWidth: 640 }}>
					Create a 1080 × 1920 image for Instagram Stories, Facebook Stories, and
					WhatsApp Status. A featured image is used as the cover when available;
					otherwise, the entry’s text fills the portrait.
				</p>
			</header>
			...
			>
				<div>
					...
					<label style={{ display: "block", color: MUTED, fontSize: 12, fontWeight: 700 }}>
					...
					{loading ? (
						...
						<p style={{ color: MUTED }}>No projects or posts are available yet.</p>
					) : (
						<>
							<EntrySection
								title="Projects"
				...
				<aside className="status-images__preview">
					...
					<p style={{ color: MUTED, fontSize: 12, lineHeight: 1.5 }}>
						The file downloads through your browser to its configured Downloads location.
					</p>
				</aside>
			</div>
		</section>
